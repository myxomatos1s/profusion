...
import './PFToken.sol';
import './PFTokenDeployer.sol';
import './interfaces/IProfusionBank.sol';
import './interfaces/IProfusionOracle.sol';

contract ProfusionBank is IProfusionBank, Initializable, Pausable {
  using Address for address;
  using SafeERC20 for IERC20;

  event Create(address indexed underlying, address pfToken);
  event Open(address indexed owner, uint indexed pid, address pfToken, address collateral);
  ...
  mapping(address => address) public pfTokens; // mapping from underlying to pfToken
  mapping(address => address) public override underlyings; // mapping from pfToken to underlying token
  ...
  modifier lock() {
    require(unlocked == 1, 'ProfusionBank/locked');
    unlocked = 2;
    _;
    unlocked = 1;
  }
  ...
  modifier onlyGov() {
    require(msg.sender == governor, 'ProfusionBank/onlyGov');
    _;
  }
  ...
  modifier isPermittedByOwner(address _owner) {
    require(isPermittedCaller(_owner, msg.sender), 'ProfusionBank/isPermittedByOwner');
    _;
  }

  /// @dev Check is pool id exist for the owner modifier.
  modifier checkPID(address _owner, uint _pid) {
    require(_pid < nextPositionIds[_owner], 'ProfusionBank/checkPID');
    _;
  }
  ...
  /// @dev Sets whether anyone can create pftoken of any token. Must only be called by the governor.
  ...
  function create(address _underlying) external lock whenNotPaused returns (address pfToken) {
    ...
    require(pfTokens[_underlying] == address(0), 'create/underlying-already-exists');
    require(IProfusionOracle(oracle).getAssetETHPrice(_underlying) > 0, 'create/no-price');
    pfToken = PFTokenDeployer(deployer).deploy(_underlying);
    pfTokens[_underlying] = pfToken;
    underlyings[pfToken] = _underlying;
    emit Create(_underlying, pfToken);
  }
  ...
  {
    Position storage pos = positions[_owner][_pid];
    return PFToken(pos.pfToken).fetchDebtShareValue(pos.debtShare);
  }
  ...
  function open(
    ...
    address pfToken = pfTokens[_underlying];
    require(pfToken != address(0), 'open/bad-underlying');
    require(_underlying != _collateral, 'open/self-collateral');
    require(IProfusionConfig(config).getCollFactor(_collateral) > 0, 'open/bad-collateral');
    require(IProfusionOracle(oracle).getAssetETHPrice(_collateral) > 0, 'open/no-price');
    pid = nextPositionIds[_owner]++;
    Position storage pos = positions[_owner][pid];
    pos.pfToken = pfToken;
    pos.collateral = _collateral;
    emit Open(_owner, pid, pfToken, _collateral);
  }
  ...
  function borrow(
    ...
    uint share = PFToken(pos.pfToken).borrow(msg.sender, _amount);
    pos.debtShare += share;
    ...
    require(ltv <= IProfusionConfig(config).getSafetyLTV(underlyings[pos.pfToken]), 'borrow/not-safe');
  ...
  function repay(
    ...
    uint share = PFToken(pos.pfToken).repay(msg.sender, _amount);
    pos.debtShare -= share;
    positions[_owner][_pid].debtShare = pos.debtShare;
  ...
  function put(
    ...
    require(
      totalCollaterals[pos.collateral] <= IProfusionConfig(config).getCollMaxAmount(pos.collateral),
  ...
  function take(
    ...
    // 3. make sure the position is still safe
    uint ltv = _fetchPositionLTV(pos);
    require(ltv <= IProfusionConfig(config).getSafetyLTV(underlyings[pos.pfToken]), 'take/not-safe');
  ...
  function liquidate(
    ...
    Position memory pos = positions[_owner][_pid];
    address underlying = underlyings[pos.pfToken];
    uint ltv = _fetchPositionLTV(pos);
    require(ltv >= IProfusionConfig(config).getLiquidationLTV(underlying), 'liquidate/not-liquidatable');
    // 2. perform repayment
    uint debtShare = PFToken(pos.pfToken).repay(msg.sender, _amount);
    require(debtShare <= (pos.debtShare + 1) / 2, 'liquidate/too-much-liquidation');
    // 3. calculate reward and payout
    uint debtValue = PFToken(pos.pfToken).fetchDebtShareValue(debtShare);
    uint collValue = IProfusionOracle(oracle).convert(underlying, pos.collateral, debtValue);
    uint payout = Math.min(
      collValue + (collValue * IProfusionConfig(config).getKillBountyRate(underlying)) / 1e18,
      pos.collateralSize
    );
  ...
  function selflessLiquidate(
    ...
    uint debtValue = PFToken(pos.pfToken).fetchDebtShareValue(pos.debtShare);
  ...
  /// @param _pfToken The PFToken to propagate this recover call.
  ...
  function recover(
    ...
    require(underlyings[_pfToken] != address(0), 'recover/not-pfToken');
    PFToken(_pfToken).recover(_token, msg.sender, _amount);
  }
  ...
  function _fetchPositionLTV(Position memory pos) internal returns (uint) {
    ...
    uint collFactor = IProfusionConfig(config).getCollFactor(pos.collateral);
    require(collFactor > 0, 'fetch/bad-collateral');
    uint debtSize = PFToken(pos.pfToken).fetchDebtShareValue(pos.debtShare);
    uint debtValue = IProfusionOracle(oracle_).getAssetETHValue(underlyings[pos.pfToken], debtSize);
    uint collCred = (pos.collateralSize * collFactor) / 1e18;
    uint collValue = IProfusionOracle(oracle_).getAssetETHValue(pos.collateral, collCred);
