...
import './interfaces/IProfusionConfig.sol';
import './interfaces/IProfusionInterestModel.sol';

contract PFToken is ERC20Permit, ReentrancyGuard {
  ...
  address public immutable profusionBank; // ProfusionBank address
  ...
  /// @dev Initializes the PFToken contract.
  /// @param _profusionBank ProfusionBank address.
  /// @param _underlying The underlying token address for the bToken.
  constructor(address _profusionBank, address _underlying)
    ERC20Permit('PF Token')
    ERC20('PF Token', 'pfTOKEN')
  {
    require(_profusionBank != address(0), 'constructor/profusionbank-zero-address');
    require(_underlying != address(0), 'constructor/underlying-zero-address');
    profusionBank = _profusionBank;
    underlying = _underlying;
    interestRate = IProfusionInterestModel(IProfusionBank(_profusionBank).interestModel()).initialRate();
  ...
  function name() public view override returns (string memory) {
    try IERC20Metadata(underlying).name() returns (string memory data) {
      return string(abi.encodePacked('PF ', data));
    } catch (bytes memory) {
      return ERC20.name();
    }
  }
  ...
  function accrue() public {
    ...
    require(!Pausable(profusionBank).paused(), 'ProfusionBank/paused');
    ...
    IProfusionConfig config = IProfusionConfig(IProfusionBank(profusionBank).config());
    IProfusionInterestModel model = IProfusionInterestModel(IProfusionBank(profusionBank).interestModel());
  ...
  /// @dev Burns the given pfToken for the proportional amount of underlying tokens.
  ...
  /// @dev Borrows the funds for the given address. Must only be called by ProfusionBank.
  ...
  function borrow(address _to, uint _amount) external nonReentrant returns (uint debtShare) {
    require(msg.sender == profusionBank, 'borrow/not-ProfusionBank');
    accrue();
  ...
  /// @dev Repays the debt using funds from the given address. Must only be called by ProfusionBank.
  ...
  function repay(address _from, uint _amount) external nonReentrant returns (uint debtShare) {
    require(msg.sender == profusionBank, 'repay/not-ProfusionBank');
    accrue();
    uint amount;
    {
      uint balBefore = IERC20(underlying).balanceOf(address(this));
  ...
  /// @dev Recovers tokens in this contract. EMERGENCY ONLY. Full trust in ProfusionBank.
  ...
  function recover(
    ...
    require(msg.sender == profusionBank, 'recover/not-ProfusionBank');
